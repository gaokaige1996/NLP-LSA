...
words.df=data.frame(lapply(words.df, function(x) as.numeric(x)))
fulldata=cbind(words.df,dataset1[,1:43])
any(is.na(fulldata))
...
best_para=function(paralist,train_sparse,labeltrain,valid_sparse,labelvalid){
  ...
  for (i in paralist){
    set.seed(1234)
    xgbmodel=xgboost(data=train_sparse,label=labeltrain,nthread=4,nrounds=100,max_depth=i,
...
xgbmodel=xgboost(data=train_sparse,label=labeltrain,nthread=4,nrounds=100,max_depth=1,
                 objective="binary:logistic")
xgbpred=predict(xgbmodel,valid_sparse)
#plot "precion recall" curves 
roc_xgb=prediction(xgbpred,labelvalid)
rp=performance(roc_xgb,"prec","rec")
plot(rp,main="rp curve xgb lsa+dataset1")
#plot roc curve
roc=performance(roc_xgb,"tpr","fpr")
roc2=performance(roc_xgb,"f","rec")
plot(roc,main="roc xgb lsa+dataset1 ")
#roc area under the curve
importance=xgb.importance(model=xgbmodel)
print(importance)
xgb.plot.importance(importance_matrix = importance)
write.csv(importance,"C:/Users/lindsay/Desktop/758W/modelling/data/out_xgb_lsa50_dataset1.csv",fileEncoding = "UTF-8")
