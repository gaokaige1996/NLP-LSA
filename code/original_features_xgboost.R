...
label=fulldata$Y1==1
#split data
index=grep("Y1",colnames(fulldata))
set.seed(1234)
...
best_para=function(paralist,train_sparse,labeltrain,valid_sparse,labelvalid){
  ...
  for (i in paralist){
    set.seed(1234)
    xgbmodel=xgboost(data=train_sparse,label=labeltrain,nthread=4,nrounds=9,max_depth=i,
...
paralist=seq(11,12,1)
result=best_para(paralist,train_sparse,labeltrain,valid_sparse,labelvalid)
result

xgbmodel=xgboost(data=train_sparse,label=labeltrain,nthread=4,nrounds=83,max_depth=12,
                 objective="binary:logistic")
xgbpred=predict(xgbmodel,valid_sparse)
...
plot(rp,main="rp curve xgb dataset1")
#plot roc curve
roc=performance(roc_xgb,"tpr","fpr")
roc2=performance(roc_xgb,"f","rec")
plot(roc,main="roc xgb dataset1 ")
...
#default
xgbmodel=xgboost(data=train_sparse,label=labeltrain,nthread=4,nrounds=9,max_depth=12,
                 objective="binary:logistic")
xgbpred=predict(xgbmodel,valid_sparse)
matrix_xg=confusion_matrix(xgbpred,labelvalid,0.5)
matrix_xg
measures=class_performance(matrix_xg)
measures
