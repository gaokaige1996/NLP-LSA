...
rf = randomForest(Y~., ntree = 500, data = train_set, mtry=3)
plot(rf)
print(rf)
# plotting the importance of each variable # mean decreaseGini higher - more important
importance(rf)
varImpPlot(rf)

#### predict Y ####
rf.pred <- predict(rf,test_set)

#### create confusion matrix ####
print(confusionMatrix(data = rf.pred, reference = test_set$Y))

#### Roc Curve ####
library(ROCR)
rf.pred = prediction(as.numeric(rf.pred), as.numeric(test_set$Y))
